...
if($input['proses']=="kirim")
{
	...
	}else
	{
		...
		$WO_MASTER_ID		=waktu_decimal(Date("Y-m-d H:i:s"));
		...
		foreach($WO_DETAIL_DESCRIPTION as $key_detail => $value_detail)
			{
				...
				$data_detail=array(
													...
													'WO_DETAIL_REMARK'			=>$input['WO_DETAIL_REMARK'][$key_detail],
...
///////////////////////////////			
				...
				$this->MYSQL=new MYSQL();
				$this->MYSQL->database=$this->CONFIG->mysql_koneksi()->db_nama;
				$this->MYSQL->queri="select WO_MASTER_ID,WO_MASTER_NOMOR,WO_MASTER_REVISI,WO_MASTER_INDEX,COMPANY_UNIT_ID,COMPANY_UNIT_SHORT_NAME from WO_MASTER where RECORD_STATUS='A' and WO_MASTER_ID='".$WO_MASTER_ID."'";				
