...
$this->MYSQL->queri="select * from WO_MASTER a, WO_LAMPIRAN_CCD b where a.WO_MASTER_ID=b.WO_MASTER_ID and a.WO_MASTER_NOMOR=b.WO_MASTER_NOMOR and a.WO_MASTER_ID='".$input['WO_MASTER_ID']."' and a.WO_MASTER_INDEX='".$input['WO_MASTER_INDEX']."' and a.RECORD_STATUS='A'";	
...
foreach($result_a as $r){
	...
	$this->MYSQL=new MYSQL();
	$this->MYSQL->database=$this->CONFIG->mysql_koneksi()->db_nama;
	$this->MYSQL->queri="select COMPANY_UNIT_ID,COMPANY_UNIT_SHORT_NAME, COMPANY_UNIT_NAME from COMPANY_UNIT WHERE  COMPANY_UNIT_ID='".$r['COMPANY_UNIT_ID_TO']."' and RECORD_STATUS='A' AND COMPANY_UNIT_TYPE='Department'";	
	...
	$this->MYSQL->queri="select 
							PERSONAL_NIK,PERSONAL_NAME, COMPANY_STRUKTUR_ORGANISASI_JABATAN_NAME,COMPANY_UNIT_SHORT_NAME 
						from WO_TEMPORARY_APPROVE
						WHERE COMPANY_UNIT_ID='".$r['COMPANY_UNIT_ID']."' and WO_MASTER_ID='".$r['WO_MASTER_ID']."' and WO_MASTER_NOMOR='".$r['WO_MASTER_NOMOR']."' and RECORD_STATUS='A'";	
	$result_approve_dept=$this->MYSQL->data();
	$r['DEPARTMENT_APPROVE']=$result_approve_dept;
	/*
	//AMBIL APPROVAL
	$this->MYSQL=new MYSQL();
	$this->MYSQL->database=$this->CONFIG->mysql_koneksi()->db_nama;
	$this->MYSQL->queri="select PERSONAL_NIK,PERSONAL_NAME from CONFIG_APPROVE_DETAIL 
		WHERE CONFIG_APPROVE_MASTER_APLIKASI_ID='wo' AND COMPANY_UNIT_ID='".$r['COMPANY_UNIT_ID']."' AND RECORD_STATUS='A' AND CONFIG_APPROVE_DETAIL_TYPE='out'";	
	...
	*/
	...
	$this->MYSQL->queri="select 
						...
						from SISTEM_APPROVE
						WHERE COMPANY_UNIT_ID='".$r['COMPANY_UNIT_ID']."' and SISTEM_APPROVE_NOREF='".$r['WO_MASTER_ID']."' and SISTEM_APPROVE_INDEXREF='".$r['WO_MASTER_INDEX']."' and RECORD_STATUS='A'";	
	$result_approve_dept=$this->MYSQL->data();
	...
	$r['CONFIG_APPROVE_DETAIL']=$result_approve_dept2;
	...
	$this->MYSQL->database=$this->CONFIG->mysql_koneksi()->db_nama;
	$this->MYSQL->queri="select PERSONAL_NIK,PERSONAL_NAME from CONFIG_APPROVE_DETAIL 
		WHERE CONFIG_APPROVE_MASTER_APLIKASI_ID='wo' AND CONFIG_APPROVE_DETAIL_TYPE='in' AND COMPANY_UNIT_ID='".$r['COMPANY_UNIT_ID_TO']."' AND RECORD_STATUS='A'";	
	...
	$this->MYSQL=new MYSQL();
	$this->MYSQL->database=$this->CONFIG->mysql_koneksi()->db_nama;
	$this->MYSQL->queri="select WO_DETAIL_INDEX,WO_DETAIL_ID,WO_DETAIL_DESCRIPTION, WO_DETAIL_QUANTITY,WO_UNIT_ID,WO_UNIT_NAME,WO_DETAIL_DATE_COMPLETED,WO_DETAIL_TUJUAN,WO_DETAIL_NOREKENING
...
for($is=0;$is<$jlh_data_tujuan;$is++)
{
	if($data_tujuan[$is]==$result_a[0]['WO_LAMPIRAN_TUJUAN'])
	{
		$cek_tujuan.='
			<tr>
				<td  width="17%">
					<input type="checkbox" class="form-check stktujuan stktujuan'.$NOBB.'" name="WO_LAMPIRAN_TUJUAN"  checked="checked" onClick=ubahTujuan('.$NOBB.') value="'.$result_a[0]['WO_LAMPIRAN_TUJUAN'].'">
				</td>
	...
	}else
	{
		$cek_tujuan.='								
					<tr>
						<td width="17%">
							<input type="checkbox" class="form-check stktujuan stktujuan'.$NOBB.'" name="WO_LAMPIRAN_TUJUAN" onClick=ubahTujuan('.$NOBB.') value="'.$data_tujuan[$is].'" disabled>
						</td>
