...
foreach($user_login['PROPERTIES']['unit'] as  $unitku){
	$unitku_filter .="OR COMPANY_UNIT_ID_TO='".$unitku['COMPANY_UNIT_ID']."' ";
}
...
{
	$filter_c="and substring(ENTRI_WAKTU,1,10)>='".Date('Y-m-d',strtotime($input['DATA_sDATE']))."'  and substring(ENTRI_WAKTU,1,10)<='".Date('Y-m-d',strtotime($input['DATA_eDATE']))."'";
}
			
//mysql query
$tabel="WO_MASTER";
//$dimana_default="where RECORD_STATUS='".$record_status_value."'  and ($unitku_filter)    $filter_b   $filter_a  $filter_c";
$dimana_default="where RECORD_STATUS='".$record_status_value."'  and ($unitku_filter)".$filter_b.$filter_c.$filter_a;
...
foreach($result_a as $r){
	...
	$this->MYSQL=new MYSQL();
	$this->MYSQL->database=$this->CONFIG->mysql_koneksi()->db_nama;
	$this->MYSQL->queri="select * from PRESENSI_LEMBUR_SPL_DETAIL where PRESENSI_LEMBUR_SPL_ID='".$r['PRESENSI_LEMBUR_SPL_ID']."' and RECORD_STATUS='".$record_status_value."'";
	$detail=$this->MYSQL->data();
	$nod=1;
	foreach($detail as $rd){
		$istirahat=$this->PRESENSI_CONFIG->hitung_jam_kerja(array('waktu_awal'=>$rd['PRESENSI_LEMBUR_SPL_DETAIL_WAKTU_BREAKOUT'],'waktu_akhir'=>$rd['PRESENSI_LEMBUR_SPL_DETAIL_WAKTU_BREAKIN']))->callback['selisihMenit'];
		$estimated=$this->PRESENSI_CONFIG->hitung_jam_kerja(array('waktu_awal'=>$rd['PRESENSI_LEMBUR_SPL_DETAIL_WAKTU_CHECKIN'],'waktu_akhir'=>$rd['PRESENSI_LEMBUR_SPL_DETAIL_WAKTU_CHECKOUT'],'pengurangan_menit_istirahat'=>$istirahat,'type'=>"lembur"))->callback;
		
		//$rd['CHECKIN']=substr($rd['PRESENSI_LEMBUR_SPL_DETAIL_WAKTU_CHECKIN'],11,5);
		//$rd['BREAKOUT']=substr($rd['PRESENSI_LEMBUR_SPL_DETAIL_WAKTU_BREAKOUT'],11,5);
		//$rd['BREAKIN']=substr($rd['PRESENSI_LEMBUR_SPL_DETAIL_WAKTU_BREAKIN'],11,5);
		//$rd['CHECKOUT']=substr($rd['PRESENSI_LEMBUR_SPL_DETAIL_WAKTU_CHECKOUT'],11,5);
		//$rd['ESTIMATED']=$estimated['total_jam_decimal'];
		$total_jam[$no] +=$estimated['total_jam_decimal'];
		$rd['NO']=$nod;
		$details[$no][]=$rd;
	$nod++;	
	}
...
if(empty($result_a)){
	...
	$this->callback['respon']['text_msg']="OK..-".$halaman."- ".$posisi."-".$batas;
	$this->callback['filter']=$params;
	$this->callback['result']=$result;
	//$this->callback['log']=$log;
