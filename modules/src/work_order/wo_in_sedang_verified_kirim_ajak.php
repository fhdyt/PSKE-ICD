...
}else
{
	...
	{	
		...
		{	
				$record_update['SISTEM_APPROVE_STATUS']="null";
				$record_update['SISTEM_APPROVE_PERSEN']="0";
				$this->MYSQL=new MYSQL();
				$this->MYSQL->database=$this->CONFIG->mysql_koneksi()->db_nama;
				$this->MYSQL->tabel="WO_MASTER";
				$this->MYSQL->record=$record_update;
				$this->MYSQL->dimana="where RECORD_STATUS='A' and WO_MASTER_ID='".$input['WO_MASTER_ID']."'";
				$this->MYSQL->ubah();
						...
						///////////////////////// NOTOFIKASI  
								...
								foreach($ck_user as $r_user){
										...
										$NOTIFICATION_DESCRIPTION		="WO No.".$ck[0]['WO_MASTER_NOMOR']." sudah diverifikasi dan diteruskan kepada ".$cek_info_approval[0]['PERSONAL_NAME'];
											...
											$NOTIFICATION_LINK_VIEW			="?show=wo/transaction/work_order/wo_out/".$ck[0]['WO_MASTER_ID']."/".$ck[0]['WO_MASTER_INDEX']."/".$ck[0]['WO_MASTER_LAMPIRAN']."/";
