...
if((isset($input['WO_MASTER_NOMOR']) and $input['WO_MASTER_NOMOR']==null) and (isset($input['WO_MASTER_ID']) and $input['WO_MASTER_ID']==null))
{
	$filter_a="WO_MASTER_LAMPIRAN_STATUS='Draft'";
	$owner_data="and ENTRI_OPERATOR='".$user_login['PERSONAL_NIK']."'"; //owner saat entri data
}else{
	$filter_a="WO_MASTER_NOMOR='".$input['WO_MASTER_NOMOR']."' AND WO_MASTER_ID='".$input['WO_MASTER_ID']."'";
	$owner_data=""; 
}
...
$this->MYSQL->queri="select * from WO_MASTER where ".$filter_a." ".$owner_data." and RECORD_STATUS='A' AND WO_MASTER_LAMPIRAN='Y'";	
...
for($is=0;$is<$jlh_data_tujuan;$is++)
{
	if($data_tujuan[$is]==$result_a[0]['WO_LAMPIRAN_TUJUAN'])
	{
		$cek_tujuan.='
			<tr>
				<td  width="17%">
					<input type="checkbox" class="form-check stktujuan stktujuan'.$NOBB.'" name="WO_LAMPIRAN_TUJUAN"  checked="checked" onClick=ubahTujuan('.$NOBB.') value="'.$result_a[0]['WO_LAMPIRAN_TUJUAN'].'" required>
				</td>
	...
	}else
	{
		$cek_tujuan.='								
					<tr>
						<td width="17%">
							<input type="checkbox" class="form-check stktujuan stktujuan'.$NOBB.'" name="WO_LAMPIRAN_TUJUAN" onClick=ubahTujuan('.$NOBB.') value="'.$data_tujuan[$is].'" required>
						</td>
		...
		$cek_prioritas.='
			<tr class="listprioritas">
				...
				<td width="10%" valign="top" class="'.$warna.' table-bordered">
					<input type="checkbox" class="form-check stkprioritas stkprioritas'.$NOTJ.'" name="WO_LAMPIRAN_PRIORITAS" checked="checked" onClick=ubahstkprioritas('.$NOTJ.') value="'.$result_a[0]['WO_LAMPIRAN_PRIORITAS'].'" required>
				</td>
			</tr>';	
	...
	}else
	{
		$cek_prioritas.='								
					<tr class="listprioritas">
						...
						<td width="2%">&nbsp;</td>
						<td width="10%" valign="top" class="'.$warna.' table-bordered">
							<input type="checkbox" class="form-check stkprioritas stkprioritas'.$NOTJ.'" name="WO_LAMPIRAN_PRIORITAS" onClick=ubahstkprioritas('.$NOTJ.') value="'.$data_prioritas[$ip].'" required>
						</td>
					</tr>
					';
	}
